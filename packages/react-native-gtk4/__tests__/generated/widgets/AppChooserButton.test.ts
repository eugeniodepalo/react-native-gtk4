import { AppChooserButton } from "@/generated/widgets.js"
...
describe("AppChooserButton", () => {
  ...
  beforeEach(() => {
    widget = new AppChooserButton(
      {},
      AppChooserButton.createNode({ contentType: "Some String" })
    )
  })
  ...
  test("should set showDefaultItem", () => {
    ...
    expect(widget.node.setShowDefaultItem).toHaveBeenCalledWith(newValue)
  })

  test("should set showDialogItem", () => {
    const newValue = true
    widget.set("showDialogItem", newValue)
    expect(widget.node.setShowDialogItem).toHaveBeenCalledWith(newValue)
  })
