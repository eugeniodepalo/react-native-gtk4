...
describe("Assistant", () => {
  ...
  beforeEach(() => {
    node = new Gtk.Assistant()
    Gtk.Assistant.mockImplementation(() => node)
  ...
  test("should connect onApply", () => {
    ...
    handler()
    expect(node.on).toHaveBeenCalledWith("apply", expect.any(Function))
  ...
  test("should connect onCancel", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(node.on).toHaveBeenCalledWith("cancel", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onEscape", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(node.on).toHaveBeenCalledWith("escape", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
  ...
  test("should connect onPrepare", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(node.on).toHaveBeenCalledWith("prepare", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
