...
describe("ComboBox", () => {
  ...
  beforeEach(() => {
    node = new Gtk.ComboBox()
    Gtk.ComboBox.mockImplementation(() => node)
    widget = new ComboBox({})
  })
  ...
  test("should set popupFixedWidth", () => {
    ...
    expect(node.setPopupFixedWidth).toHaveBeenCalledWith(newValue)
  })
  ...
  test("should connect onMoveActive", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(node.on).toHaveBeenCalledWith("move-active", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
