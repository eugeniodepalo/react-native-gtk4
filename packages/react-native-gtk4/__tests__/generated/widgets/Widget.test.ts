import { Widget } from "@/generated/widgets.js"
...
describe("Widget", () => {
  ...
  beforeEach(() => {
    widget = new Widget({}, Widget.createNode({ cssName: "Some String" }))
  })
  ...
  test("should connect onDestroy", () => {
    ...
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("destroy", expect.any(Function))
  ...
  test("should connect onRealize", () => {
    ...
    expect(handler).toBeDefined()
    handler()
    expect(widget.node.on).toHaveBeenCalledWith("realize", expect.any(Function))
    expect(callback).toHaveBeenCalled()
  })
